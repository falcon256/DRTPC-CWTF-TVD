//Original code from: https://github.com/mattatz/unity-volume-rendering
//Modified my Dan to have color and look cool instead of render fricking white bunnies.
...
Shader "VolumeRendering"
{
	SubShader 
	{
		...
		Pass
		{
			...
			float4 SampleVolume(float3 uv, float3 p)
			{
				...
				return tex3D(_Volume, uv) * _Intensity * min * max;
			}
			...
			fixed4 SetPixelShader(in float4 vertex : POSITION, in float3 world:TEXCOORD1) : SV_Target
			{
				...
				for (int i = 0; i < 150; i++)
				{
					float4 s = SampleVolume(ro+0.5, ro);
